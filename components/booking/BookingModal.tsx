...
Timeline: ${displayDate} at ${form.time}`;
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "relative z-10 w-full flex flex-col bg-background border border-amber-500/20",
          "shadow-[0_0_120px_rgba(215,165,32,0.12),0_32px_80px_rgba(0,0,0,0.2)]",
      ...
      >
        ...
        {step === 1 && (
          <div className="flex flex-col h-full overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-4 sm:px-6 pt-3 pb-2 border-b border-border/50 shrink-0">
              {/* Drag handle (mobile) */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-base font-bold text-foreground leading-tight">Book a Strategic Session</h2>
                  <p className="text-[11px] text-amber-500/80 font-semibold tracking-wider uppercase">Quick qualification · 30 sec</p>
                </div>
              </div>
              <button
                onClick={closeBookingModal}
                className="w-8 h-8 rounded-full bg-secondary hover:bg-secondary/80 flex items-center justify-center transition-colors text-muted-foreground hover:text-foreground"
              >
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-3 space-y-2.5 sm:space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-10 h-10 bg-secondary/50 border-border rounded-xl text-foreground placeholder:text-muted-foreground font-medium focus-visible:border-amber-500/60 focus-visible:ring-0 focus-visible:ring-offset-0 transition-colors"
                    autoComplete="name"
                  />
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                <Label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground">
                  Company Name <span className="text-muted-foreground/70 capitalize tracking-normal font-medium">(Optional)</span>
                </Label>
                <div className="relative">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={form.topic} onValueChange={(val) => setForm((f) => ({ ...f, topic: val }))}>
                  <SelectTrigger className="h-10 bg-secondary/50 border-border rounded-xl text-foreground font-medium focus:ring-0 focus:ring-offset-0 data-[placeholder]:text-muted-foreground focus:border-amber-500/60 transition-colors [&>span]:truncate">
                  ...
                  <SelectContent className="bg-popover border-border rounded-xl shadow-2xl">
                    {BOOKING_TOPICS.map((topic) => (
                      <SelectItem
                        key={topic}
                        value={topic}
                        className="text-foreground/80 font-medium focus:bg-amber-500/10 focus:text-amber-600 dark:focus:text-amber-400 rounded-lg cursor-pointer"
                      >
                        {topic}
                      </SelectItem>
                    ))}
            ...
            {/* CTA Footer */}
            <div className="px-4 sm:px-6 pb-3 pt-2.5 border-t border-border/50 shrink-0">
              <Button
                onClick={() => setStep(2)}
                disabled={!isValid}
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="flex items-center gap-3 px-4 sm:px-6 py-3 border-b border-border/50 shrink-0">
              ...
              <div className="flex items-center gap-2 min-w-0">
                <span className="text-[11px] font-black uppercase tracking-widest text-amber-500 shrink-0">Booking for</span>
                <span className="text-sm font-bold text-foreground truncate">{form.name}</span>
              </div>
              <button
                onClick={closeBookingModal}
                className="ml-auto w-7 h-7 rounded-full bg-secondary hover:bg-secondary/80 flex items-center justify-center transition-colors text-muted-foreground hover:text-foreground shrink-0"
              >
            ...
            <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-6 space-y-6">
              <div className="flex flex-col sm:flex-row gap-6 sm:gap-8">
                ...
                <div className="space-y-3 flex-1 flex flex-col w-full sm:max-w-[380px] mx-auto sm:mx-0">
                  <Label className="text-[11px] font-black uppercase tracking-widest text-muted-foreground text-center sm:text-left">
                    Select Date <span className="text-amber-500">*</span>
                  </Label>
                  <div className="p-2 sm:p-3 bg-secondary/50 border border-border rounded-xl flex justify-center w-full">
                    <Calendar
                      mode="single"
                      ...
                      className="text-foreground bg-transparent pointer-events-auto"
                      ...
                      classNames={{
                        ...
                        day: "h-9 w-9 sm:h-11 sm:w-11 p-0 font-normal hover:bg-secondary hover:text-foreground rounded-md transition-all text-xs sm:text-sm",
                        cell: "h-9 w-9 sm:h-11 sm:w-11 text-center p-0 relative",
                        nav_button_previous: "absolute left-0 sm:left-1 w-7 h-7 flex items-center justify-center hover:bg-secondary hover:text-foreground rounded-md transition-all",
                        nav_button_next: "absolute right-0 sm:right-1 w-7 h-7 flex items-center justify-center hover:bg-secondary hover:text-foreground rounded-md transition-all",
                        head_cell: "text-muted-foreground w-9 sm:w-11 font-medium text-[0.7rem] sm:text-[0.8rem] uppercase tracking-wider",
                        caption: "flex justify-center pt-1 pb-2 relative items-center text-sm font-bold text-foreground gap-1 px-8",
                        ...
                        dropdown: "bg-popover border border-border text-foreground text-xs sm:text-sm rounded-md px-1 sm:px-2 py-1 focus:ring-1 focus:ring-amber-500/50 outline-none cursor-pointer hover:bg-secondary transition-colors [&>option]:bg-popover [&>option]:text-foreground [&>option]:py-1",
                      }}
                    />
                  </div>
                </div>
                      ...
                      <button
                        ...
                        className={cn(
                          ...
                          form.time === time 
                            ...
                            : "bg-secondary/50 border-border text-foreground/70 hover:border-amber-500/50 hover:text-foreground hover:bg-secondary"
                        )}
                      >
                        {time}
                      </button>
            ...
            {/* CTA Footer */}
            <div className="px-4 sm:px-6 pb-4 pt-3 border-t border-border/50 shrink-0">
              <Button
        ...
        {step === 3 && (
          <div className="flex flex-col h-full overflow-hidden items-center justify-center py-12 px-6 text-center">
            ...
            <h2 className="text-2xl font-bold text-foreground mb-3">Request Successful!</h2>
            <p className="text-sm text-muted-foreground mb-8 max-w-sm mx-auto leading-relaxed">
              We have received your booking details for <strong className="text-foreground font-medium">{form.topic}</strong> on <strong className="text-foreground font-medium">{form.date}</strong> at <strong className="text-foreground font-medium">{form.time}</strong>.
            </p>
...
Timeline: ${displayDate} at ${form.time}`;
              ...
              <Button
                variant="ghost"
                onClick={closeBookingModal}
                className="w-full h-12 text-muted-foreground hover:text-foreground hover:bg-secondary rounded-xl font-medium transition-all"
              >
                Close Window
              </Button>
            </div>
